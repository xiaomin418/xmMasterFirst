...
class Active_set(object):
    ...
    def find_active_set(self):
        ...
        while True:
            ...
            # 计算在可行解处的导数,作为新的c
            partial_x = self.calculate_delta(feasible_x)#求Aλ=Gx+g
            actual_A, actual_b, actual_b1 = find_new_data(self.A, self.b, activate_set_rows)#获得有效集对应的约束A,b1
            # 利用Lagrange求得等式约束的解
...
if __name__ == "__main__":
    H = np.array([[1,-4,2,1], [-4,16,-8,-4],[2,-8,4,2],[1,-4,2,1]])
    c = np.array([-1,0,7,4]).reshape(-1, 1)
    A = np.array([[1,1,1,1]])
